  ...
  public function getAll(){
    if(isset($_GET['searchquery'])){ //search by patrol dailyid
        if(preg_match("/^[a-zA-Z0-9 ]+$/", $_GET['searchquery'])){
            $get_all_unconfirmed_patrols = DB::table('daily')->where('code',$_GET['searchquery'])->get();
            return view('show-unconfirmed-patrols',['patrols'=>$get_all_unconfirmed_patrols]);
        } else {
            $get_all_unconfirmed_patrols = DB::table('daily')->get();
            return view('show-unconfirmed-patrols',['patrols'=>$get_all_unconfirmed_patrols]);
        }
        ...
        if(preg_match("/^[A-Za-z0-9]+$/", $_GET['patrol'])){
            ...
            $patrol_id = DB::table('daily')->where('code',$official_code_of_patrol)->first()->iddaily;
            ...
            $last_table_infos_getting_iddaily;
            if(DB::table('patrol')->where('fuel_type','gasoline')->where('annex_id',$get_annex_id)->where('iddaily',$patrol_id)->first()->iddaily){
            ...
            }elseif(DB::table('patrol')->where('fuel_type','diesel')->where('annex_id',$get_annex_id)->where('iddaily',$patrol_id)->first()->iddaily){
            ...
            }elseif(DB::table('patrol')->where('fuel_type','essence91')->where('annex_id',$get_annex_id)->where('iddaily',$patrol_id)->first()->iddaily){
            ...
            }elseif(DB::table('patrol')->where('fuel_type','essence95')->where('annex_id',$get_annex_id)->where('iddaily',$patrol_id)->first()->iddaily){
